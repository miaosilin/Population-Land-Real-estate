...
#读取数据
datafile = u'C:\\Users\\miaosilin\\Desktop\\Population_land_realestate\\PLR_do\\bubble_data\\bubble_alldata.xlsx'
...
colors = ['white','indianred','firebrick','lightcoral','rosybrown']

size=data['Coupling Coordination Degree'].rank()
n=9
plt.scatter(data['Coordination Degree'],data['Coupling Degree'])
#定义一个字典，将颜色跟对应的分类进行绑定
color={1:'red',2:'indianred',3:'lightcoral',4:'rosybrown'}
#增加color的参数，用列表解析式将data分类中的每个数据的数字映射到前面color的颜色中
plt.scatter(data['Coordination Degree'],data['Coupling Degree'],color=[color[i] for i in data['type']])

for i in range(166):
    plt.annotate(data['city'][i], xy = (data['Coordination Degree'][i], data['Coupling Degree'][i]), \
                 xytext = (data['Coordination Degree'][i]-0.028, data['Coupling Degree'][i]-0.003),fontsize=9)
    # 这里xy是需要标记的坐标，xytext是对应的标签坐标
    
plt.savefig('bubbleall.pdf',dpi=600,format='pdf')
plt.savefig('bubbleall.jpg',dpi=600,format='jpg')
plt.show()#显示图像
