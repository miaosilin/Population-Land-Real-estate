...
#读取数据
datafile = u'C:\\Users\\miaosilin\\Desktop\\Population_land_realestate\\PLR_do\\bubble_data\\bubble_data5.xlsx'
data = pd.read_excel(datafile)
 
plt.figure(figsize=(10,5))#设置画布的尺寸
plt.title('Urban Agglomeration on the West Side of the Straits')
...
for i in range(18):
    plt.annotate(data['city'][i], xy = (data['Coordination Degree'][i], data['Coupling Degree'][i]), \
                 xytext = (data['Coordination Degree'][i]-0.01, data['Coupling Degree'][i]-0.003))
    # 这里xy是需要标记的坐标，xytext是对应的标签坐标
plt.savefig('bubble5.pdf',dpi=600,format='pdf')
plt.savefig('bubble5.jpg',dpi=600,format='jpg')
plt.show()#显示图像
