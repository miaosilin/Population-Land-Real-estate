...
#读取数据
datafile = u'C:\\Users\\miaosilin\\Desktop\\Population_land_realestate\\PLR_do\\bubble_data\\bubble_data8.xlsx'
data = pd.read_excel(datafile)
 
plt.figure(figsize=(10,5))#设置画布的尺寸
plt.title('Shandong Peninsula Urban Agglomerations')
...
for i in range(8):
    plt.annotate(data['city'][i], xy = (data['Coordination Degree'][i], data['Coupling Degree'][i]), \
                 xytext = (data['Coordination Degree'][i]-0.009, data['Coupling Degree'][i]-0.003))
    # 这里xy是需要标记的坐标，xytext是对应的标签坐标
plt.savefig('bubble8.pdf',dpi=600,format='pdf')
plt.savefig('bubble8.jpg',dpi=600,format='jpg')
plt.show()#显示图像
